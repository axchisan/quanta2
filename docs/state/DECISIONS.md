...
- **2026-06-17 — ui-web (T027):** Quark reacciona en la partida multiplayer en vivo (`/sala`):
  `thinking` durante la pregunta, `correct`/`wrong` en el reveal según la respuesta del jugador.
  Reuso del personaje (avatar-system-spec §7), sin deps. Hace sentir viva la sala.

- **2026-06-17 — ui-web (T026):** Accesibilidad de respuestas: las 4 opciones de `/sala` llevan
  **color + forma** (▲◆●■, `components/answer-shape.tsx`), nunca color solo (patrón Kahoot, ayuda a
  daltónicos). Color por índice con utilidades estáticas (`bg-primary/info/streak/success`, del design
  system v2). El reveal resalta la correcta (✓) y atenúa el resto. Sin deps nuevas.
...
- **2026-06-15 — ui-web (T017):** Cerrado el dossier de benchmark de diseño (`docs/research/design-benchmark/`):
  ...
  Se hizo con 8 subagentes en paralelo (uno por sitio). **Limitación:** WebSearch/WebFetch denegados →
  análisis desde conocimiento documentado (HEX/timings aproximados, marcados); el script
  `tools/extract-design-tokens.mjs` queda para un pase con datos en vivo. **Recomendación técnica clave
  (avatares):** evolucionar el `AtomMascot` (hoy SVG estático) a **Rive con State Machine** (un `.riv` con
  input `state=idle|thinking|correct|wrong|celebrate|streakLost`), con **Lottie/canvas-confetti solo para
  efectos one-shot**, y una **Fase 0 puente** en SVG/CSS para enviar rápido. Coleccionables **ganados por
  puntos** (ético, no gacha de pago). El pulido se deriva en tasks de Fase B (no se codea en T017).

- **2026-06-15 — backend/web (T023):** Personalización de partida: la cantidad de preguntas y los
  segundos por pregunta son configurables por sala (opciones `count` y `questionSeconds` en `create`).
  El server clampa (`count` [1,10], `questionSeconds` [5,60]) y usa `this.questionMs` por instancia en
  vez de la constante global `QUESTION_MS` (afecta `deadline` y el `timeBonus`). El timer del cliente ya
  era server-driven (lee `deadline`), así que no necesitó cambios. UI: selectores en `/sala`.
